...
func (f *FeeCreditModule) checkTransferFC(tx *types.TransactionOrder, attr *fc.AddFeeCreditAttributes, exeCtx txtypes.ExecutionContext) (*fc.TransferFeeCreditAttributes, error) {
	...
	if transProof.PartitionID() != f.moneyPartitionID {
		return nil, fmt.Errorf("invalid transferFC money partition identifier %d (expected %d)", transProof.PartitionID(), f.moneyPartitionID)
	}
