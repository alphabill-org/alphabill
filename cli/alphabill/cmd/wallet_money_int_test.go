...
func TestSendingMoneyUsingWallets_integration(t *testing.T) {
	...
	apiAddr := startMoneyBackend(t, nodeAddr, initialBill)
	...
	stdout := execWalletCmd(t, homedir1, fmt.Sprintf("fees add --amount %d --alphabill-api-uri %s --alphabill-uri %s", feeAmountAlpha, apiAddr, nodeAddr))
