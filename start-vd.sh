...
build/alphabill root-genesis --home testab/rootchain -p testab/vd1/vd/node-genesis.json -p testab/vd2/vd/node-genesis.json -p testab/vd3/vd/node-genesis.json -k testab/rootchain/keys.json -g
...
for i in 1 2 3
do
  build/alphabill vd --home testab/vd$i -f testab/vd$i/vd/blocks.db -k testab/vd$i/vd/keys.json -r "/ip4/127.0.0.1/tcp/26662" -a "/ip4/127.0.0.1/tcp/$port" --server-address ":$grpcPort" -g testab/rootchain/rootchain/partition-genesis-1.json -p "$nodeAddresses" > "testab/vd$i/log.log" &
  ((port=port+1))
  ((grpcPort=grpcPort+1))
done
