...
func initConsensusManager(t *testing.T, db keyvaluedb.KeyValueDB) (*ConsensusManager, *testutils.TestNode, []*testutils.TestNode, *genesis.RootGenesis) {
	...
	require.NoError(t, err)
	cm, err := NewMonolithicConsensusManager(rootNode.PeerConf.ID.String(), rootGenesis, partitions, rootNode.Signer, testlogger.New(t).With(logger.NodeID(id)), consensus2.WithStorage(db))
	require.NoError(t, err)
...
func TestConsensusManager_NormalOperation(t *testing.T) {
	...
	req := consensus2.IRChangeRequest{
