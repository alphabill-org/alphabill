...
func TestNode_LedgerReplicationRequestTimeout(t *testing.T) {
	...
	ir := &types.InputRecord{
		...
		Hash:         test2.RandomBytes(32),
		BlockHash:    test2.RandomBytes(32),
	...
	require.Eventually(t, RequestReceived(tp, network.ProtocolLedgerReplicationReq), ledgerReplicationTimeout+time.Second, test2.WaitTick)
}
...
func TestNode_HandleUnicityCertificate_RevertAndStartRecovery_withNoProposal(t *testing.T) {
	...
	rootRound++
	partitionRound++
	newStateHash := test2.RandomBytes(32)
	ir := &types.InputRecord{
		PreviousHash: bl.UnicityCertificate.InputRecord.Hash,
		Hash:         newStateHash,
		BlockHash:    test2.RandomBytes(32),
...
func TestNode_RecoverySimulateStorageFailsDuringBlockFinalizationOnUC(t *testing.T) {
	...
	require.Eventually(t, RequestReceived(tp, network.ProtocolHandshake), 200*time.Millisecond, test2.WaitTick)
	...
	}, test2.WaitDuration, test2.WaitTick, "block certification request not found")
