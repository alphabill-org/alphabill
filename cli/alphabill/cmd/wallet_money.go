...
func execCollectDust(cmd *cobra.Command, config *walletConfig) error {
	...
	for _, dcResult := range dcResults {
		if dcResult.SwapProof != nil {
			...
			msg := "Dust collection finished successfully on account #%d. Joined %d bills into target bill with total value of %s ALPHA."
			consoleWriter.Println(fmt.Sprintf(msg, dcResult.AccountNumber, len(attr.DcTransfers), amountToString(attr.TargetValue, 8)))
		} else {
