...
import (
	...
	"github.com/alphabill-org/alphabill/wvm"
)
...
func Call(ctx context.Context, wasm []byte, fName string, execCtx wvm.ExecutionCtx, storage wvm.Storage, log *slog.Logger) (err error) {
	// todo: AB-1006 automatic revert of changes when program execution fails,
	abHostMod, err := wvm.BuildABHostModule(execCtx, log, storage)
	if err != nil {
...
func CheckProgram(ctx context.Context, wasm []byte, execCtx wvm.ExecutionCtx, log *slog.Logger) error {
	// check wasm source, init wasm VM
	abHostMod, err := wvm.BuildABHostModule(execCtx, log, wvm.NewMemoryStorage())
	if err != nil {
		return fmt.Errorf("failed to intialize ab host module, %w", err)
	}
	vm, err := wvm.New(ctx, wasm, wvm.WithHostModule(abHostMod))
