...
func (v *VoteRegister) InsertVote(vote *abdrc.VoteMsg, quorumInfo QuorumInfo) (*abtypes.QuorumCert, error) {
	if vote == nil {
	...
	if uint32(len(quorum.signatures)) >= quorumInfo.GetQuorumThreshold() {
		qc := abtypes.NewQuorumCertificateFromVote(quorum.voteInfo, quorum.commitInfo, quorum.signatures)
		return qc, nil
	}
...
func (v *VoteRegister) InsertTimeoutVote(timeout *abdrc.TimeoutMsg, quorumInfo QuorumInfo) (*abtypes.TimeoutCert, error) {
	...
	if v.timeoutCert == nil {
		v.timeoutCert = &abtypes.TimeoutCert{
			...
			Signatures: make(map[string]*abtypes.TimeoutVote),
		}
	}
	// append signature
