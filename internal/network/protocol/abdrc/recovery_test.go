	...
	t.Run("commit head block is from later round", func(t *testing.T) {
		sm := &StateMsg{
			CommittedHead: &RecoveryBlock{
				Block:    &abdrc.BlockData{Round: 5},
				CommitQc: &abdrc.QuorumCert{VoteInfo: &abdrc.RoundInfo{RoundNumber: 7}},
			},
		}
	...
	t.Run("most common case", func(t *testing.T) {
		sm := &StateMsg{
			...
			BlockNode: []*RecoveryBlock{
				{
					Block: &abdrc.BlockData{Round: 6},
					Qc:    &abdrc.QuorumCert{VoteInfo: &abdrc.RoundInfo{RoundNumber: 7}},
				},
				{
					Block: &abdrc.BlockData{Round: 7},
				},
			},
		}
	...
	t.Run("commit head, commit QC is nil", func(t *testing.T) {
		sm := &StateMsg{
			...
			CommittedHead: &RecoveryBlock{
				...
				Block: &abdrc.BlockData{
					Round:   5,
					Payload: &abdrc.Payload{},
					Qc: &abdrc.QuorumCert{
						VoteInfo: &abdrc.RoundInfo{
							RoundNumber:       4,
							ParentRoundNumber: 3,
