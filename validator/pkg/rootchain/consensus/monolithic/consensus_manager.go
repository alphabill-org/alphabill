...
import (
	...
	consensus2 "github.com/alphabill-org/alphabill/validator/pkg/rootchain/consensus"
	"github.com/alphabill-org/alphabill/validator/pkg/rootchain/partitions"
	"github.com/alphabill-org/alphabill/validator/pkg/rootchain/unicitytree"
)

type (
	ConsensusManager struct {
		certReqCh    chan consensus2.IRChangeRequest
		certResultCh chan *types.UnicityCertificate
		params       *consensus2.Parameters
...
func NewMonolithicConsensusManager(selfStr string, rg *genesis.RootGenesis, partitionStore partitions.PartitionConfiguration,
	signer crypto.Signer, log *slog.Logger, opts ...consensus2.Option) (*ConsensusManager, error) {
	verifier, err := signer.Verifier()
	if err != nil {
		return nil, fmt.Errorf("signing key error, %w", err)
	}
	// load optional parameters
	optional := consensus2.LoadConf(opts)
	// Initiate store
	...
	consensusParams := consensus2.NewConsensusParams(rg.Root)
	consensusManager := &ConsensusManager{
		certReqCh:    make(chan consensus2.IRChangeRequest),
...
func (x *ConsensusManager) RequestCertification() chan<- consensus2.IRChangeRequest {
	return x.certReqCh
}
	...
	for {
		...
		case req, ok := <-x.certReqCh:
			...
			if err := x.onIRChangeReq(&req); err != nil {
				x.log.WarnContext(ctx, "handling certification request", logger2.Error(err))
			}
		// handle timeouts
		case <-ticker.C:
			x.onT3Timeout(ctx)
...
func (x *ConsensusManager) onT3Timeout(ctx context.Context) {
	...
	if err != nil {
		x.log.WarnContext(ctx, "T3 timeout failed", logger2.Round(newRound), logger2.Error(err))
		return
	}
	...
	for id, cert := range certs {
		x.log.DebugContext(ctx, fmt.Sprintf("sending new UC for '%s'", id), logger2.Round(newRound))
		select {
...
func (x *ConsensusManager) checkT2Timeout(round uint64) error {
	log := x.log.With(logger2.Round(round))
	// evaluate timeouts
	for id := range x.ir {
		...
		if _, found := x.changes[id]; !found {
			...
			if err != nil {
				log.Warn(fmt.Sprintf("read certificate for %s", id), logger2.Error(err))
				continue
			}
...
func getMergeInputRecords(currentIR, changed map[types.SystemID32]*types.InputRecord, log *slog.Logger) map[types.SystemID32]*types.InputRecord {
	...
	for id, ch := range changed {
		...
		log.LogAttrs(context.Background(), logger2.LevelTrace, fmt.Sprintf("Partition %s IR", id), logger2.Data(ch))
	}
	return result
}
...
func (x *ConsensusManager) generateUnicityCertificates(round uint64) (map[types.SystemID32]*types.UnicityCertificate, error) {
	...
	if len(x.changes) == 0 {
		x.log.Info("no IR changes", logger2.Round(round))
		// persist new round
	...
	for sysID, ir := range x.changes {
		...
		x.log.LogAttrs(context.Background(), logger2.LevelTrace, fmt.Sprintf("NewStateStore unicity certificate for partition %s is", sysID), logger2.Data(uc))
		certs[sysID] = uc
	}
	// persist new state
