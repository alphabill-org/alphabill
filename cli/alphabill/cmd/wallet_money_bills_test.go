...
func TestWalletBillsExportCmd_BillIdFlag(t *testing.T) {
	t.Skip("AB-666")
	homedir := createNewTestWallet(t)
	mockServer, addr := mockBackendCalls(&backendMockReturnConf{customPath: "/" + client.ProofPath, customResponse: fmt.Sprintf(`{"bills": [{"id":"%s","value":"%d","txHash":"MHgwMzgwMDNlMjE4ZWVhMzYwY2JmNTgwZWJiOTBjYzhjOGNhZjBjY2VmNGJmNjYwZWE5YWI0ZmMwNmI1YzM2N2IwMzg=","is_dc_bill":false}]}`, toBase64(money.NewBillID(nil, []byte{1})), 1)})
	defer mockServer.Close()
...
func TestWalletBillsExportCmd_ShowUnswappedFlag(t *testing.T) {
	...
	mockServer, addr := mockBackendCalls(&backendMockReturnConf{
		...
		customFullPath: "/" + client.ListBillsPath + "?pubkey=" + pubKey + "&includeDcBills=false",
		customResponse: `{"total": 1, "bills": [{"id":"` + toBase64(money.NewBillID(nil, []byte{2})) + `","value":"22222222"}]}`})
	...
	mockServer, addr = mockBackendCalls(&backendMockReturnConf{
		...
		customFullPath: "/" + client.ListBillsPath + "?pubkey=" + pubKey + "&includeDcBills=true",
		customResponse: `{"total": 1, "bills": [{"id":"` + toBase64(money.NewBillID(nil, []byte{3})) + `","value":"33333333"}]}`})
