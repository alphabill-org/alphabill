...
import (
	...
	"github.com/alphabill-org/alphabill/common/debug"
...
type (
	...
	Node struct {
		...
		net              PartitionNet
		consensusManager consensus2.Manager
		log              *slog.Logger
	}
)
...
func (v *Node) loop(ctx context.Context) error {
	for {
		...
		case msg, ok := <-v.net.ReceivedChannel():
			...
			v.log.LogAttrs(ctx, logger2.LevelTrace, fmt.Sprintf("received %T", msg), logger2.Data(msg))
			switch mt := msg.(type) {
			case *certification.BlockCertificationRequest:
				if err := v.onBlockCertificationRequest(ctx, mt); err != nil {
					v.log.LogAttrs(ctx, slog.LevelWarn, fmt.Sprintf("handling block certification request from %s", mt.NodeIdentifier), logger2.Error(err))
				}
			case *handshake.Handshake:
				if err := v.onHandshake(ctx, mt); err != nil {
					v.log.LogAttrs(ctx, slog.LevelWarn, fmt.Sprintf("handling handshake from %s", mt.NodeIdentifier), logger2.Error(err))
				}
			default:
...
func (v *Node) onBlockCertificationRequest(ctx context.Context, req *certification.BlockCertificationRequest) error {
	...
	err = consensus2.CheckBlockCertificationRequest(req, latestUnicityCertificate)
	if err != nil {
	...
	case QuorumAchieved:
		...
		select {
		case v.consensusManager.RequestCertification() <- consensus2.IRChangeRequest{
			SystemIdentifier: sysID32,
			Reason:           consensus2.Quorum,
			Requests:         proof,
		}:
		case <-ctx.Done():
			return ctx.Err()
	...
	case QuorumNotPossible:
		...
		case v.consensusManager.RequestCertification() <- consensus2.IRChangeRequest{
			SystemIdentifier: sysID32,
			Reason:           consensus2.QuorumNotPossible,
			Requests:         proof,
		}:
		case <-ctx.Done():
			return ctx.Err()
...
func (v *Node) onCertificationResult(ctx context.Context, certificate *types.UnicityCertificate) {
	...
	if err != nil {
		v.log.WarnContext(ctx, "failed to send certification result", logger2.Error(err))
		return
	}
	...
	defer func() {
		...
		v.log.LogAttrs(ctx, logger2.LevelTrace, fmt.Sprintf("Resetting request store for partition '%s'", sysID))
	}()
	...
	for _, node := range subscribed {
		if err := v.sendResponse(ctx, node, certificate); err != nil {
			v.log.WarnContext(ctx, "sending certification result", logger2.Error(err))
