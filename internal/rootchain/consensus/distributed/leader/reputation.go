...
type BlockLoader func(round uint64) (*ab_consensus.BlockData, error)
...
*/
func (rb *ReputationBased) Update(qc *ab_consensus.QuorumCert, currentRound uint64) error {
...
func (rb *ReputationBased) electLeader(qc *ab_consensus.QuorumCert) (peer.ID, error) {
	qcRound := qc.GetRound()
	...
	for i := 0; i < rb.windowSize || len(authors) < rb.excludeSize; i++ {
		...
		if len(authors) < rb.excludeSize {
			authors[block.GetAuthor()] = struct{}{}
		}
		if i < rb.windowSize {
			for signerID := range qc.GetSignatures() {
