...
func (m *LockTokensModule) handleLockTokenTx() txsystem.GenericExecuteFunc[LockTokenAttributes] {
	return func(tx *types.TransactionOrder, attr *LockTokenAttributes, ctx *txsystem.TxExecutionContext) (*types.ServerMetadata, error) {
		...
		updateFn := state.UpdateUnitData(tx.UnitID(),
			func(data state.UnitData) (state.UnitData, error) {
				return m.updateLockTokenData(data, tx, attr, ctx.CurrentBlockNr)
			})
