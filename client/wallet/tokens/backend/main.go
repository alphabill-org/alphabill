...
import (
	...
	"github.com/alphabill-org/alphabill/common/abclient"
	...
	"github.com/alphabill-org/alphabill/client/wallet/blocksync"
	"github.com/alphabill-org/alphabill/client/wallet/broker"
...
func Run(ctx context.Context, cfg Configuration) error {
	...
	abc, err := cfg.Client()
	if err != nil {
		return fmt.Errorf("failed to create Alphabill abclient: %w", err)
	}
...
type cfg struct {
	abc      abclient.AlphabillClientConfig
	boltDB   string
	apiAddr  string
	log      *slog.Logger
	systemID []byte
...
func NewConfig(apiAddr, abURL, boltDB string, logger *slog.Logger, systemID []byte) Configuration {
	return &cfg{
		abc:      abclient.AlphabillClientConfig{Uri: abURL},
		boltDB:   boltDB,
		apiAddr:  apiAddr,
		log:      logger,
		systemID: systemID,
...
func (c *cfg) Client() (ABClient, error) { return abclient.New(c.abc, c.log) }
