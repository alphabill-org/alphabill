...
func InfoEndpoints(node partitionNode, name string, self *network2.Peer, log *slog.Logger) RegistrarFunc {
	return func(r *mux.Router) {
		r.HandleFunc("/info", infoHandler(node, name, self, log)).Methods(http.MethodGet, http.MethodOptions)
	}
}

func infoHandler(node partitionNode, name string, self *network2.Peer, log *slog.Logger) http.HandlerFunc {
...
func getPartitionValidators(self *network2.Peer) []peerInfo {
...
func getOpenConnections(self *network2.Peer) []peerInfo {
...
func getRootValidators(self *network2.Peer, log *slog.Logger) []peerInfo {
	var peers []peerInfo
	peerStore := self.Network().Peerstore()
	ids := peerStore.Peers()
	for _, id := range ids {
		protocols, err := peerStore.SupportsProtocols(id, network2.ProtocolBlockCertification)
		if err != nil {
			log.Warn("failed to query peer store", logger.Error(err))
			continue
		}
		if slices.Contains(protocols, network2.ProtocolBlockCertification) {
...
func getBootstrapNodes(self *network2.Peer) []peerInfo {
