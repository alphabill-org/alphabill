...
import (
	...
	logger2 "github.com/alphabill-org/alphabill/common/logger"
	util2 "github.com/alphabill-org/alphabill/common/util"
	"github.com/alphabill-org/alphabill/txsystem"
	"github.com/alphabill-org/alphabill/validator/pkg/metrics"
	network2 "github.com/alphabill-org/alphabill/validator/pkg/network"
	...
	"github.com/alphabill-org/alphabill/validator/pkg/network/protocol/handshake"
	"github.com/alphabill-org/alphabill/validator/pkg/network/protocol/replication"
	"github.com/alphabill-org/alphabill/validator/pkg/partition/event"
	pgenesis "github.com/alphabill-org/alphabill/validator/pkg/partition/genesis"
	"github.com/alphabill-org/alphabill/validator/pkg/txbuffer"
...
func NewNode(
	ctx context.Context,
	peerConf *network2.PeerConfiguration,
...
func (n *Node) Run(ctx context.Context) error {
	...
	g.Go(func() error {
		err := n.loop(ctx)
		n.log.DebugContext(ctx, "node main loop exit", logger2.Error(err))
		return err
	})
...
func (n *Node) GetPeer() *network2.Peer {
	return n.peer
}
...
func (n *Node) initNetwork(ctx context.Context, peerConf *network2.PeerConfiguration) (err error) {
	n.peer, err = network2.NewPeer(ctx, peerConf, n.log)
	if err != nil {
		return err
	}
	...
	n.network, err = network2.NewLibP2PValidatorNetwork(n.peer, network2.DefaultValidatorNetOptions, n.log)
	if err != nil {
		return err
	}
...
func (n *Node) sendHandshake(ctx context.Context) {
	...
	if err := n.network.Send(ctx,
		...
		n.log.ErrorContext(ctx, "error sending handshake", logger2.Error(err))
	}
}
...
func (n *Node) applyBlockTransactions(round uint64, txs []*types.TransactionRecord) (txsystem.State, uint64, error) {
	...
	for _, tx := range txs {
		...
		if err != nil {
			n.log.Warn("processing transaction", logger2.Error(err), logger2.UnitID(tx.TransactionOrder.UnitID()))
...
func (n *Node) restoreBlockProposal(prevBlock *types.Block) {
	...
	if err != nil {
		n.log.Error("Error fetching block proposal", logger2.Error(err))
		return
	}
	if !found {
	...
	if err != nil {
		n.log.Warn("Block proposal recovery failed", logger2.Error(err))
		n.revertState()
		return
	}
...
func (n *Node) loop(ctx context.Context) error {
	...
	for {
		...
		case m, ok := <-n.network.ReceivedChannel():
			...
			n.log.Log(ctx, logger2.LevelTrace, fmt.Sprintf("received %T", m), logger2.Data(m))
			switch mt := m.(type) {
			case *types.TransactionOrder:
				if err := n.handleTxMessage(mt); err != nil {
					n.log.Warn("invalid transaction", logger2.Error(err), logger2.UnitID(mt.UnitID()))
			...
			case *types.UnicityCertificate:
				if err := n.handleUnicityCertificate(ctx, mt); err != nil {
					n.log.Warn("Unicity Certificate processing failed", logger2.Error(err))
					n.sendEvent(event.Error, err)
				}
...
func (n *Node) process(tx *types.TransactionOrder, round uint64) error {
	...
	n.log.Debug(fmt.Sprintf("transaction processed, proposal size: %d", len(n.proposedTransactions)), logger2.UnitID(tx.UnitID()))
	return nil
}
...
func (n *Node) updateLUC(uc *types.UnicityCertificate) error {
	...
	if n.luc.CompareAndSwap(luc, uc) {
		n.log.Debug("updated LUC", logger2.Round(uc.GetRoundNumber()))
...
func (n *Node) startNewRound(ctx context.Context, uc *types.UnicityCertificate) error {
	...
	if n.status.Load() == recovering {
		n.log.InfoContext(ctx, "node is recovered", logger2.Round(uc.InputRecord.RoundNumber))
	...
	if err := n.blockStore.Delete(util2.Uint32ToBytes(proposalKey)); err != nil {
		n.log.DebugContext(ctx, "DB proposal delete failed", logger2.Error(err))
	}
...
func (n *Node) startRecovery(ctx context.Context, uc *types.UnicityCertificate) {
	...
	if err := n.updateLUC(uc); err != nil {
		n.log.WarnContext(ctx, "Start recovery unicity certificate update failed", logger2.Error(err))
		return
	}
	luc := n.luc.Load()
	if n.status.Load() == recovering {
...
func (n *Node) handleUnicityCertificate(ctx context.Context, uc *types.UnicityCertificate) error {
	...
	if err := types.CheckNonEquivocatingCertificates(luc, uc); err != nil {
		...
		n.log.WarnContext(ctx, fmt.Sprintf("equivocating UC for round %d", uc.InputRecord.RoundNumber), logger2.Error(err), logger2.Data(uc))
		n.log.WarnContext(ctx, "LUC", logger2.Data(luc))
		return fmt.Errorf("equivocating certificate: %w", err)
	}

	if uc.GetRootRoundNumber() == luc.GetRootRoundNumber() {
		n.log.DebugContext(ctx, "duplicate UC (same root round)", logger2.Round(uc.GetRootRoundNumber()))
...
func (n *Node) handleT1TimeoutEvent(ctx context.Context) {
	...
	if err := n.sendBlockProposal(ctx); err != nil {
		n.log.WarnContext(ctx, "Failed to send BlockProposal", logger2.Error(err))
		return
	}
	if err := n.sendCertificationRequest(ctx, n.peer.ID().String()); err != nil {
		n.log.WarnContext(ctx, "Failed to send certification request", logger2.Error(err))
	}
}
	...
	go func() {
		...
		defer func() {
			if err := dbIt.Close(); err != nil {
				n.log.WarnContext(ctx, "closing DB iterator", logger2.Error(err))
			}
		}()
		...
		for ; dbIt.Valid(); dbIt.Next() {
			...
			roundNo := util2.BytesToUint64(dbIt.Key())
			if err := dbIt.Value(&bl); err != nil {
				n.log.WarnContext(ctx, fmt.Sprintf("Ledger replication reply incomplete, failed to read block %d", roundNo), logger2.Error(err))
				break
			}
			lastFetchedBlock = &bl
		...
		if err := n.sendLedgerReplicationResponse(ctx, resp, lr.NodeIdentifier); err != nil {
			n.log.WarnContext(ctx, fmt.Sprintf("Problem sending ledger replication response, %s", resp.Pretty()), logger2.Error(err))
		}
	}()
	return nil
}
...
func (n *Node) handleLedgerReplicationResponse(ctx context.Context, lr *replication.LedgerReplicationResponse) error {
	...
	onError := func(latestSuccessfulRoundNumber uint64, err error) error {
		// log problems
		n.log.ErrorContext(ctx, "Recovery failed", logger2.Error(err))
	...
	if n.recoveryLastProp != nil {
		...
		if err = n.handleBlockProposal(ctx, n.recoveryLastProp); err != nil {
			n.log.DebugContext(ctx, "Recovery completed, failed to apply last received block proposal(stale?)", logger2.Error(err))
		}
...
func (n *Node) sendLedgerReplicationRequest(ctx context.Context, startingBlockNr uint64) {
	...
	n.log.Log(ctx, logger2.LevelTrace, "sending ledger replication request", logger2.Data(req))
	...
	for _, p := range util2.ShuffleSliceCopy(peers) {
		...
		if err := n.network.Send(ctx, req, p); err != nil {
			n.log.DebugContext(ctx, "Error sending ledger replication request", logger2.Error(err))
			continue
		}
...
func (n *Node) sendBlockProposal(ctx context.Context) error {
	...
	n.log.Log(ctx, logger2.LevelTrace, "created BlockProposal", logger2.Data(prop))
...
func (n *Node) sendCertificationRequest(ctx context.Context, blockAuthor string) error {
	...
	n.log.Log(ctx, logger2.LevelTrace, "Block Certification req", logger2.Data(req))
...
func (n *Node) txProcessorForRound(round uint64, leader peer.ID) txbuffer.TxHandler {
	log := n.log.With(logger2.Round(round))
	...
	case n.peer.ID():
		return func(ctx context.Context, tx *types.TransactionOrder) {
			if err := n.process(tx, round); err != nil {
				log.WarnContext(ctx, "processing transaction", logger2.Error(err), logger2.UnitID(tx.UnitID()))
			}
		}
	default:
		return func(ctx context.Context, tx *types.TransactionOrder) {
			msg := fmt.Sprintf("forward tx %X to %v", tx.Hash(n.configuration.hashAlgorithm), leader)
			log.DebugContext(ctx, msg, logger2.UnitID(tx.UnitID()))
			if err := n.network.Send(ctx, tx, leader); err != nil {
				log.WarnContext(ctx, "failed to "+msg, logger2.Error(err), logger2.UnitID(tx.UnitID()))
			}
		}
	}
}
