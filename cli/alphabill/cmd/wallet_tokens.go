...
func tokenCmdNewTokenFungible(config *walletConfig) *cobra.Command {
	...
	cmd.Flags().String(cmdFlagAmount, "", "amount value, must be bigger than 0 and match type precision (decimals)")
...
func execTokenCmdListTypes(cmd *cobra.Command, config *walletConfig, kind t.TokenKind) error {
	...
	for _, tok := range res {
		consoleWriter.Println(fmt.Sprintf("ID=%X, symbol=%s, kind: %#v", tok.ID, tok.Symbol, tok.Kind))
	}
	return nil
}
