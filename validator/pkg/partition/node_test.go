...
import (
	...
	test2 "github.com/alphabill-org/alphabill/validator/pkg/testutils"
	"github.com/alphabill-org/alphabill/validator/pkg/testutils/logger"
	"github.com/alphabill-org/alphabill/validator/pkg/testutils/network"
	...
	"github.com/alphabill-org/alphabill/validator/pkg/testutils/txsystem"
...
func TestNode_NodeStartTest(t *testing.T) {
	...
	require.Eventually(t, RequestReceived(tp, network.ProtocolHandshake), 200*time.Millisecond, test2.WaitShortTick)
	...
	}, test2.WaitDuration, test2.WaitTick)
}
...
func TestNode_HandleOlderUnicityCertificate(t *testing.T) {
	...
	require.NoError(t, tp.SubmitTx(transfer))
	tp.CreateBlock(t)
	require.Eventually(t, NextBlockReceived(t, tp, block), test2.WaitDuration, test2.WaitTick)
...
func TestNode_HandleEquivocatingUnicityCertificate_SameRoundDifferentIRHashes(t *testing.T) {
	...
	ir.Hash = test2.RandomBytes(32)
	ir.BlockHash = test2.RandomBytes(32)
...
func TestBlockProposal_Ok(t *testing.T) {
	...
	require.NoError(t, err)
	tp.SubmitBlockProposal(bp)
	require.Eventually(t, RequestReceived(tp, network.ProtocolBlockCertification), test2.WaitDuration, test2.WaitTick)
}
...
func TestNode_GetTransactionRecord_NotFound(t *testing.T) {
	...
	record, proof, err := tp.partition.GetTransactionRecord(context.Background(), test2.RandomBytes(32))
