...
func TestNewPeer_InvalidPrivateKey(t *testing.T) {
	conf := &PeerConfiguration{
		KeyPair: &PeerKeyPair{
			PrivateKey: test2.RandomBytes(30),
		},
	}
...
func TestNewPeer_InvalidPublicKey(t *testing.T) {
	...
	conf := &PeerConfiguration{
		KeyPair: &PeerKeyPair{
			...
			PublicKey:  test2.RandomBytes(30),
		},
	}
	_, err := NewPeer(context.Background(), conf, logger.New(t))
...
func TestBootstrapNodes(t *testing.T) {
	...
	require.Eventually(t, func() bool { return peer1.dht.RoutingTable().Size() == 1 }, test2.WaitDuration, test2.WaitTick)
	...
	require.Eventually(t, func() bool { return peer2.dht.RoutingTable().Size() == 2 }, test2.WaitDuration, test2.WaitTick)
	require.Eventually(t, func() bool { return peer1.dht.RoutingTable().Size() == 2 }, test2.WaitDuration, test2.WaitTick)
	require.Eventually(t, func() bool { return peer2.dht.RoutingTable().Find(peer1.dht.Host().ID()) != "" }, test2.WaitDuration, test2.WaitTick)
	require.Eventually(t, func() bool { return peer1.dht.RoutingTable().Find(peer2.dht.Host().ID()) != "" }, test2.WaitDuration, test2.WaitTick)
}
