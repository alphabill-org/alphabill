...
func Test_ReputationBased(t *testing.T) {
	...
	blockStore := map[uint64]*abtypes.BlockData{}
	// to save the block to blockStore
	storeBlock := func(bd *abtypes.BlockData) error {
	...
	processRound := func(rl *ReputationBased, round uint64) error {
		...
		qc := &abtypes.QuorumCert{
			VoteInfo:   &abtypes.RoundInfo{RoundNumber: round - 1, ParentRoundNumber: round - 2},
