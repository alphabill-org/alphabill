...
type TxSystem struct {
	...
	executors           txsystem2.TxExecutors
	genericTxValidators []txsystem2.GenericTransactionValidator
...
func NewEVMTxSystem(systemIdentifier []byte, log *slog.Logger, opts ...Option) (*TxSystem, error) {
	...
	txs := &TxSystem{
		...
		executors:           make(map[string]txsystem2.TxExecutor),
		genericTxValidators: []txsystem2.GenericTransactionValidator{evm.GenericTransactionValidator(), fees.GenericTransactionValidator()},
		log:                 log,
	}
...
func (m *TxSystem) StateSummary() (txsystem2.State, error) {
	if !m.state.IsCommitted() {
		return nil, txsystem2.ErrStateContainsUncommittedChanges
	}
	return m.getState()
}

func (m *TxSystem) getState() (txsystem2.State, error) {
	sv, hash, err := m.state.CalculateRoot()
	if err != nil {
		return nil, err
	}
	if hash == nil {
		return txsystem2.NewStateSummary(make([]byte, m.hashAlgorithm.Size()), util.Uint64ToBytes(sv)), nil
	}
	return txsystem2.NewStateSummary(hash, util.Uint64ToBytes(sv)), nil
}
...
func (m *TxSystem) Execute(tx *types.TransactionOrder) (sm *types.ServerMetadata, err error) {
	...
	ctx := &txsystem2.TxValidationContext{
		Tx:               tx,
		Unit:             u,
...
func (m *TxSystem) EndBlock() (txsystem2.State, error) {
