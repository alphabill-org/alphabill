...
import (
	...
	partitions2 "github.com/alphabill-org/alphabill/validator/pkg/rootchain/partitions"
	"github.com/alphabill-org/alphabill/validator/pkg/rootchain/testutils"
	...
	"github.com/alphabill-org/alphabill/validator/pkg/testutils/peer"
...
type MockConsensusManager struct {
	...
	partitions   partitions2.PartitionConfiguration
	certs        map[types.SystemID32]*types.UnicityCertificate
}

func NewMockConsensus(rg *genesis.RootGenesis, partitionStore partitions2.PartitionConfiguration) (*MockConsensusManager, error) {
...
func initRootValidator(t *testing.T, net PartitionNet) (*Node, *testutils.TestNode, []*testutils.TestNode, *genesis.RootGenesis) {
	...
	require.NoError(t, err)
	partitionStore, err := partitions2.NewPartitionStoreFromGenesis(rootGenesis.Partitions)
	require.NoError(t, err)
