...
func TestAddFC_ValidateAddNewFeeCreditTx(t *testing.T) {
	...
	t.Run("ok - empty", func(t *testing.T) {
		...
		require.NoError(t, feeCreditModule.validateAddFC(tx, &attr, &authProof, testctx.NewMockExecutionContext(t, testctx.WithCurrentRound(5))))
	})
	t.Run("err - replay will not pass validation", func(t *testing.T) {
		...
		sm, err := feeCreditModule.executeAddFC(tx, &attr, &authProof, testctx.NewMockExecutionContext(t, testctx.WithCurrentRound(10)))
		require.NoError(t, err)
		require.NotNil(t, sm)
		// replay attack
	...
	t.Run("transferFC tx order is nil", func(t *testing.T) {
		...
		authProof := fcsdk.AddFeeCreditAuthProof{OwnerProof: testsig.NewOwnerProof(t, tx, signer)}
		require.NoError(t, tx.SetAuthProof(authProof))
		feeCreditModule := newTestFeeModule(t, trustBase)
		execCtx := testctx.NewMockExecutionContext(t, testctx.WithCurrentRound(5))
		var attr fcsdk.AddFeeCreditAttributes
...
func Test_addFeeCreditTxAndUpdate(t *testing.T) {
			...
			testfc.WithTransferFCRecord(
				&types.TransactionRecord{
					TransactionOrder: testfc.NewTransferFC(t, signer, testfc.NewTransferFCAttr(t, signer, testfc.WithAmount(100), testfc.WithTargetRecordID(privKeyHash), testfc.WithTargetSystemID(evm.DefaultSystemID)),
						testtransaction.WithSystemID(0x00000001), testtransaction.WithAuthProof(fcsdk.TransferFeeCreditAuthProof{OwnerProof: templates.NewP2pkh256BytesFromKeyHash(pubHash)})),
	...
	require.NoError(t, feeCreditModule.validateAddFC(addFeeOrder, attr, authProof, testctx.NewMockExecutionContext(t, testctx.WithCurrentRound(5))))
	metaData, err := feeCreditModule.executeAddFC(addFeeOrder, attr, authProof, testctx.NewMockExecutionContext(t, testctx.WithCurrentRound(5)))
	require.NoError(t, err)
