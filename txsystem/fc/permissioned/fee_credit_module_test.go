...
func TestNewFeeCreditModule(t *testing.T) {
	...
	t.Run("missing system id", func(t *testing.T) {
		m, err := NewFeeCreditModule(0, stateTree, feeCreditRecordUnitType, adminOwnerPredicate)
		require.Nil(t, m)
		require.ErrorIs(t, err, ErrMissingSystemIdentifier)
	})

	t.Run("state is nil", func(t *testing.T) {
		m, err := NewFeeCreditModule(systemID, nil, feeCreditRecordUnitType, adminOwnerPredicate)
		require.Nil(t, m)
		require.ErrorIs(t, err, ErrStateIsNil)
	})

	t.Run("fee credit record unit type is nil", func(t *testing.T) {
		m, err := NewFeeCreditModule(systemID, stateTree, nil, adminOwnerPredicate)
		require.Nil(t, m)
	...
	t.Run("ok", func(t *testing.T) {
		m, err := NewFeeCreditModule(systemID, stateTree, feeCreditRecordUnitType, adminOwnerPredicate)
		require.NoError(t, err)
		require.NotNil(t, m)
