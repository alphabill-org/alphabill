...
func TestRpcServer_GetBlocksOk(t *testing.T) {
	...
	res, err := p.GetBlocks(context.Background(), &alphabill2.GetBlocksRequest{BlockNumber: 1, BlockCount: 12})
	require.NoError(t, err)
...
func TestRpcServer_GetBlocksSingleBlock(t *testing.T) {
	...
	require.NoError(t, err)
	res, err := p.GetBlocks(context.Background(), &alphabill2.GetBlocksRequest{BlockNumber: 1, BlockCount: 1})
	require.NoError(t, err)
...
func TestRpcServer_GetBlocksMostlyEmpty(t *testing.T) {
	...
	require.NoError(t, err)
	res, err := p.GetBlocks(context.Background(), &alphabill2.GetBlocksRequest{BlockNumber: 1, BlockCount: 50})
	require.NoError(t, err)
	require.Len(t, res.Blocks, 5)
	// BatchMaxBlockNumber is really BatchMaxRoundNumber or BatchEnd, including empty blocks,
	// such that next request from abclient should be BatchEnd + 1.
...
func TestRpcServer_FetchNonExistentBlocks_DoesNotPanic(t *testing.T) {
	...
	require.NoError(t, err)
	res, err := p.GetBlocks(context.Background(), &alphabill2.GetBlocksRequest{BlockNumber: 73, BlockCount: 100})
	require.NoError(t, err)
...
func TestRpcServer_ProcessTransaction_Fails(t *testing.T) {
	...
	defer con.Close()
	response, err := client.ProcessTransaction(ctx, &alphabill2.Transaction{Order: createTransactionOrder(t, failingUnitID)})
...
func TestRpcServer_ProcessTransaction_Ok(t *testing.T) {
	...
	response, err := client.ProcessTransaction(ctx, &alphabill2.Transaction{Order: req})
	assert.Nil(t, err)
	assert.NotNil(t, response)
}

func createRpcClient(t *testing.T, ctx context.Context) (*grpc.ClientConn, alphabill2.AlphabillServiceClient) {
	t.Helper()
	listener := bufconn.Listen(1024 * 1024)
	grpcServer := grpc.NewServer()
	rpcServer, _ := NewGRPCServer(&MockNode{})
	alphabill2.RegisterAlphabillServiceServer(grpcServer, rpcServer)
	go func() {
	...
	return conn, alphabill2.NewAlphabillServiceClient(conn)
}
