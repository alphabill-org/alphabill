...
func NewDummyVoteInfo(round uint64, rootHash []byte) *abtypes.RoundInfo {
	return &abtypes.RoundInfo{
		RoundNumber:       round,
		Epoch:             0,
		Timestamp:         1111,
...
func NewDummyTc(round uint64, qc *abtypes.QuorumCert) *abtypes.TimeoutCert {
...
func TestVoteRegister_Tc(t *testing.T) {
	...
	qcRound2 := abtypes.NewQuorumCertificate(NewDummyVoteInfo(2, []byte{0, 1, 2}), nil)
	qcRound2.Signatures = qcSignatures
	qcRound3 := abtypes.NewQuorumCertificate(NewDummyVoteInfo(3, []byte{0, 1, 3}), nil)
	...
	tc, voteCnt, err = register.InsertTimeoutVote(timeoutVoteMsg, quorumInfo)
	require.ErrorContains(t, err, `failed to add vote to timeout certificate: node1 already voted`)
	require.Nil(t, tc)
