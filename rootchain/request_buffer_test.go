...
func TestCertRequestStore_isConsensusReceived_TwoNodes(t *testing.T) {
	...
	trustBase := partitions.NewPartitionTrustBase(map[string]crypto.Verifier{"1": nil, "2": nil})
	// 1.
	res, proof, err := cs.Add(&certification.BlockCertificationRequest{Partition: sysID1, NodeID: "1", InputRecord: ir1}, trustBase)
	require.NoError(t, err)
	require.Equal(t, QuorumInProgress, res)
	require.Nil(t, proof)
	// 2.
	res, proof, err = cs.Add(&certification.BlockCertificationRequest{Partition: sysID1, NodeID: "2", InputRecord: ir2}, trustBase)
	require.NoError(t, err)
	require.Equal(t, QuorumNotPossible, res)
	require.NotNil(t, proof)
	require.Len(t, proof, 2)
	...
	require.Equal(t, QuorumInProgress, res)
	require.Nil(t, proof)
	res, proof, err = cs.Add(&certification.BlockCertificationRequest{Partition: sysID1, NodeID: "2", InputRecord: ir1}, trustBase)
	require.NoError(t, err)
	require.Equal(t, QuorumAchieved, res)
	require.NotNil(t, proof)
	require.Len(t, proof, 2)
...
func TestCertRequestStore_isConsensusReceived_MultiplePartitionIds(t *testing.T) {
	...
	require.Equal(t, QuorumInProgress, res)
	require.Nil(t, proof)
	res, proof, err = cs.Add(&certification.BlockCertificationRequest{Partition: sysID2, NodeID: "1", InputRecord: ir2}, trustBase)
	require.NoError(t, err)
	require.Equal(t, QuorumInProgress, res)
	require.Nil(t, proof)
	...
	res, proof, err = cs.Add(&certification.BlockCertificationRequest{Partition: sysID2, NodeID: "2", InputRecord: ir2}, trustBase)
	require.NoError(t, err)
	require.Equal(t, QuorumAchieved, res)
	require.NotNil(t, proof)
