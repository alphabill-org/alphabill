...
func NewValidatorTrustBase(publicKeyInfo []*PublicKeyInfo) (map[string]crypto2.Verifier, error) {
	...
	nodeIdToKey := make(map[string]crypto2.Verifier)
	for _, info := range publicKeyInfo {
		ver, err := crypto2.NewVerifierSecp256k1(info.SigningPublicKey)
		if err != nil {
			return nil, err
		}
...
func (x *PublicKeyInfo) IsValid() error {
	...
	if _, err := crypto2.NewVerifierSecp256k1(x.SigningPublicKey); err != nil {
		return fmt.Errorf("invalid signing key, %w", err)
	}
	if len(x.EncryptionPublicKey) == 0 {
		return ErrPubKeyInfoEncryptionIsInvalid
	}
	if _, err := crypto2.NewVerifierSecp256k1(x.EncryptionPublicKey); err != nil {
