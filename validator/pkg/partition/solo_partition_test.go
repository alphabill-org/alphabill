...
func (sn *SingleNodePartition) IssueBlockUC(t *testing.T) *types.UnicityCertificate {
	req := sn.mockNet.SentMessages(network2.ProtocolBlockCertification)[0].Message.(*certification.BlockCertificationRequest)
	sn.mockNet.ResetSentMessages(network2.ProtocolBlockCertification)
...
func (sn *SingleNodePartition) SubmitT1Timeout(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return len(sn.mockNet.SentMessages(network2.ProtocolBlockCertification)) == 1
...
func createPeerConfiguration(t *testing.T) *network2.PeerConfiguration {
	...
	peerConf, err := network2.NewPeerConfiguration(
		"/ip4/127.0.0.1/tcp/0",
		&network2.PeerKeyPair{PublicKey: pubKeyBytes, PrivateKey: privKeyBytes},
		nil,
