...
func TestModule_validateSwapTx(t *testing.T) {
	...
	t.Run("InvalidDcProofInvalid", func(t *testing.T) {
		...
		module := newTestMoneyModule(t, verifier,
			...
			withStateUnit(DustCollectorMoneySupplyID, &money.BillData{Value: 1e8, Counter: 0}))
		exeCtx := testctx.NewMockExecutionContext()
		require.EqualError(t, module.validateSwapTx(swapTx, swapAttr, authProof, exeCtx), "dust transfer proof is not valid at index 0: invalid unicity certificate: verifying unicity seal: verifying signatures: quorum not reached, signed_votes=0 quorum_threshold=1")
	})
