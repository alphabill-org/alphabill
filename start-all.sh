...
for i in 1 2 3
do
  build/alphabill money --home testab/money$i -f testab/money$i/money/blocks.db -k testab/money$i/money/keys.json -r "/ip4/127.0.0.1/tcp/26662" -a "/ip4/127.0.0.1/tcp/$moneyPort" --server-address ":$moneyGrpcPort" -g testab/rootchain/rootchain/partition-genesis-0.json -p "$moneyNodeAddresses" > "testab/money$i/money$i.log" &
  ((moneyPort=moneyPort+1))
...
for i in 1 2 3
do
  build/alphabill vd --home testab/vd$i -f testab/vd$i/vd/blocks.db -k testab/vd$i/vd/keys.json -r "/ip4/127.0.0.1/tcp/26662" -a "/ip4/127.0.0.1/tcp/$vdPort" --server-address ":$vdGrpcPort" -g testab/rootchain/rootchain/partition-genesis-1.json -p "$vdNodeAddresses" > "testab/vd$i/vd$i.log" &
  ((vdPort=vdPort+1))
