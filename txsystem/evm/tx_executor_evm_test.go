...
func BenchmarkCallContract(b *testing.B) {
	...
	_, err = Execute(1, stateDB, blockDB, &evm.TxAttributes{
		...
		Gas:   10000000,
		Value: big.NewInt(0),
	}, nil, partitionIdentifier, gasPool, gasPrice, false, log)
	require.NoError(b, err)
	...
	b.Run("call counter contract", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			if _, err = Execute(2, stateDB, blockDB, callContract, nil, partitionIdentifier, gasPool, gasPrice, false, log); err != nil {
...
func initStateDBWithAccountAndSC(t *testing.T, accounts []*testAccount) *statedb.StateDB {
	...
	for _, eoa := range accounts {
		...
		if len(eoa.Code) != 0 {
			...
			evm := vm.NewEVM(blockCtx, NewTxContext(evmAttr, big.NewInt(0)), stateDB, NewChainConfig(new(big.Int).SetBytes(partitionIdentifier.Bytes())), NewVMConfig())
...
func Test_execute(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			blockDB, err := memorydb.New()
			require.NoError(t, err)
			metadata, err := Execute(tt.args.currentBlockNumber, tt.args.stateDB, blockDB, tt.args.attr, nil, partitionIdentifier, tt.args.gp, gasPrice, false, logger.New(t))
			if tt.wantErrStr != "" {
				require.ErrorContains(t, err, tt.wantErrStr)
...
func Test_ReplayContractCreation(t *testing.T) {
	...
	blockDB, err := memorydb.New()
	require.NoError(t, err)
	metadata, err := Execute(1, stateDB, blockDB, evmAttr, nil, partitionIdentifier, gasPool, gasPrice, false, log)
	require.NoError(t, err)
	...
	// Try to replay
	_, err = Execute(1, stateDB, blockDB, evmAttr, nil, partitionIdentifier, gasPool, gasPrice, false, log)
...
func Test_ReplayCall(t *testing.T) {
	...
	require.NoError(t, err)
	log := logger.New(t)
	metadata, err := Execute(2, stateDB, blockDB, callContract, nil, partitionIdentifier, gasPool, gasPrice, false, log)
	require.NoError(t, err)
	require.NotNil(t, metadata)
	// check that fee and account balance add up to initial value
	...
	// try to replay
	_, err = Execute(2, stateDB, blockDB, callContract, nil, partitionIdentifier, gasPool, gasPrice, false, log)
	require.ErrorContains(t, err, "nonce too low")
}
...
func Test_PreviousBlockHashFunction(t *testing.T) {
	...
	require.NoError(t, err)
	b := &types.Block{
		Header:             &types.Header{PartitionID: evm.DefaultPartitionID},
	...
	res, err := Execute(2, stateDB, mockDB, callContract, nil, partitionIdentifier, gasPool, gasPrice, false, log)
	require.NoError(t, err)
	...
	callContract.Nonce++
	res, err = Execute(3, stateDB, mockDB, callContract, nil, partitionIdentifier, gasPool, gasPrice, false, log)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, types.TxStatusSuccessful, res.SuccessIndicator)
	require.NoError(t, types.Cbor.Unmarshal(res.ProcessingDetails, &details))
