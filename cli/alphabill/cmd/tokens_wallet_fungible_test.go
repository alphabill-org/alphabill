...
func TestFungibleToken_Subtyping_Integration(t *testing.T) {
	...
	execTokensCmd(t, homedirW1, fmt.Sprintf("new-type fungible -u %s --symbol %s --type %X --subtype-clause 0x53510087", backendUrl, symbol1, typeID11))
	...
	execTokensCmdWithError(t, homedirW1, fmt.Sprintf("new-type fungible -u %s --symbol %s --type %X --subtype-clause %s --subtype-input %s", backendUrl, symbol1, typeID12, "ptpkh", "0x535100"), "missing [parent-type]")
	//inheriting the first one and setting subtype clause to ptpkh
	execTokensCmd(t, homedirW1, fmt.Sprintf("new-type fungible -u %s --symbol %s --type %X --subtype-clause %s --parent-type %X --subtype-input %s", backendUrl, symbol1, typeID12, "ptpkh", typeID11, "0x535100"))
	require.Eventually(t, testpartition.BlockchainContains(partition, func(tx *txsystem.Transaction) bool {
...
func TestFungibleToken_InvariantPredicate_Integration(t *testing.T) {
	...
	typeID11 := randomID(t)
	typeID12 := randomID(t)
	execTokensCmd(t, homedirW1, fmt.Sprintf("new-type fungible -u %s  --symbol %s --type %X --decimals 0 --inherit-bearer-clause %s", backendUrl, symbol1, typeID11, predicatePtpkh))
	require.Eventually(t, testpartition.BlockchainContains(partition, func(tx *txsystem.Transaction) bool {
		return bytes.Equal(tx.UnitId, typeID11)
	}), test.WaitDuration, test.WaitTick)
	ensureTokenTypeIndexed(t, ctx, client, w1key.PubKey, typeID11)
...
func TestWalletCreateFungibleTokenTypeAndTokenAndSendCmd_IntegrationTest(t *testing.T) {
	...
	execTokensCmdWithError(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 00.0.00", backendUrl, typeID), fmt.Sprintf("more than one comma"))
	execTokensCmdWithError(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount .00", backendUrl, typeID), fmt.Sprintf("missing integer part"))
	execTokensCmdWithError(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount a.00", backendUrl, typeID), fmt.Sprintf("invalid amount string"))
	execTokensCmdWithError(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 0.0a", backendUrl, typeID), fmt.Sprintf("invalid amount string"))
	execTokensCmdWithError(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 1.1111", backendUrl, typeID), fmt.Sprintf("invalid precision"))
	// out of range because decimals = 3 the value is equal to 18446744073709551615000
	execTokensCmdWithError(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 18446744073709551615", backendUrl, typeID), fmt.Sprintf("out of range"))
	// creation succeeds
	execTokensCmd(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 3", backendUrl, typeID))
	execTokensCmd(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 1.1", backendUrl, typeID))
	execTokensCmd(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 1.11", backendUrl, typeID))
	execTokensCmd(t, homedir, fmt.Sprintf("new fungible  -u %s --type %X --amount 1.111", backendUrl, typeID))
	...
	execTokensCmdWithError(t, homedir, fmt.Sprintf("send fungible -u %s --type %X --amount .00 --address 0x%X -k 1", backendUrl, typeID, w2key.PubKey), fmt.Sprintf("missing integer part"))
	execTokensCmdWithError(t, homedir, fmt.Sprintf("send fungible -u %s --type %X --amount a.00 --address 0x%X -k 1", backendUrl, typeID, w2key.PubKey), fmt.Sprintf("invalid amount string"))
	execTokensCmdWithError(t, homedir, fmt.Sprintf("send fungible -u %s --type %X --amount 1.1111 --address 0x%X -k 1", backendUrl, typeID, w2key.PubKey), fmt.Sprintf("invalid precision"))
}
