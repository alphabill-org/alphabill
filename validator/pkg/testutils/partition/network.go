...
import (
	...
	partition2 "github.com/alphabill-org/alphabill/validator/pkg/partition"
	...
	"github.com/alphabill-org/alphabill/validator/pkg/testutils/partition/event"
...
type partitionNode struct {
	*partition2.Node
	...
	confOpts     []partition2.NodeOption
	AddrGRPC     string
...
type rootNode struct {
	*rootchain.Node
	EncKeyPair *network2.PeerKeyPair
...
func (r *RootPartition) start(ctx context.Context) error {
	...
	peerConf, err := network2.NewPeerConfiguration(fmt.Sprintf("/ip4/127.0.0.1/tcp/%v", port), r.Nodes[0].EncKeyPair, nil, nil)
	if err != nil {
		return fmt.Errorf("failed to create peer configuration: %w", err)
	}

	rootPeer, err := network2.NewPeer(ctx, peerConf, r.log)
	if err != nil {
		return fmt.Errorf("failed to create new peer node: %w", err)
	}
	log := r.log.With(logger.NodeID(rootPeer.ID()))
	rootNet, err := network2.NewLibP2PRootChainNetwork(rootPeer, 100, 300*time.Millisecond, log)
	if err != nil {
...
func NewPartition(t *testing.T, nodeCount int, txSystemProvider func(trustBase map[string]crypto2.Verifier) txsystem.TransactionSystem, systemIdentifier []byte) (abPartition *NodePartition, err error) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		nodeGenesis, err := partition2.NewNodeGenesis(
			txSystemProvider(map[string]crypto2.Verifier{"genesis": nil}),
			partition2.WithPeerID(peerConf.ID),
			partition2.WithSigningKey(signer),
			partition2.WithEncryptionPubKey(peerConf.KeyPair.PublicKey),
			partition2.WithSystemIdentifier(systemIdentifier),
			partition2.WithT2Timeout(2500),
		)
		if err != nil {
...
func (n *NodePartition) start(t *testing.T, ctx context.Context, rootID peer.ID, rootAddr multiaddr.Multiaddr) error {
	...
	for _, nd := range n.Nodes {
		...
		nd.confOpts = append(nd.confOpts, partition2.WithRootAddressAndIdentifier(rootAddr, rootID),
			partition2.WithEventHandler(nd.EventHandler.HandleEvent, 100),
			partition2.WithBlockStore(blockStore),
			partition2.WithTxIndexer(txIndexer),
		)
...
func createPeerConfs(ctx context.Context, count int) ([]*network2.PeerConfiguration, error) {
	var peerConfs = make([]*network2.PeerConfiguration, count)
	...
	for i := 0; i < count; i++ {
		peerConfs[i], err = network2.NewPeerConfiguration(
			"/ip4/127.0.0.1/tcp/0",
...
func generateKeyPairs(count int) ([]*network2.PeerKeyPair, error) {
	var keyPairs = make([]*network2.PeerKeyPair, count)
	for i := 0; i < count; i++ {
		...
		keyPairs[i] = &network2.PeerKeyPair{
