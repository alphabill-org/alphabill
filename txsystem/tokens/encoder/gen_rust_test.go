	...
	*/
	...
	t.Run("CreateNonFungibleTokenTypeAttributes", func(t *testing.T) {
		...
		for n, tc := range tests {
			require.NoError(t, txo.Payload.SetAttributes(tc.attr))
	...
	t.Run("NonFungibleTokenData", func(t *testing.T) {
		tests := []utEnc{
			...
			},
			{
				...
				enc: []byte{0x1, 0x1, 0x1, 0x0, 0x0, 0x0, 0x1, 0x2, 0x4, 0x9, 0x0, 0x0, 0x0, 0x68, 0x6f, 0x74, 0x20, 0x73, 0x74, 0x75, 0x66, 0x66, 0x3, 0x4, 0x7, 0x0, 0x0, 0x0, 0x66, 0x6f, 0x6f, 0x2f, 0x62, 0x61, 0x72, 0x4, 0x1, 0x3, 0x0, 0x0, 0x0, 0x9, 0x1, 0x1, 0x5, 0x2, 0x20, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x6, 0x2, 0x5a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7, 0x2, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0},
			},
		}
		for n, tc := range tests {
...
func Test_generateTxAttributeDecodeTests(t *testing.T) {
	...
	*/
	...
	generateTest := func(t *testing.T, encoder func(txo *types.TransactionOrder, ver uint32) ([]byte, error), version uint32, typeName string, tests []inOut) {
		...
		out.WriteString("// test generated by Go backend!\n")
		for _, td := range tests {
			require.NoError(t, txo.Payload.SetAttributes(td.attr))
