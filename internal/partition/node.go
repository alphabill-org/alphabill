...
// Partition genesis can be generated by using NewNodeGenesis and NewPartitionGenesis functions.
func New(
	...
	genesis *genesis.PartitionGenesis, // partition genesis file
...
func (p *Node) GetCurrentProposal() []*transaction.Transaction {
	return p.proposal
}

func (p *Node) startNewRound(uc *certificates.UnicityCertificate) {
...
func (p *Node) handleTxEvent(event interface{}) {
	switch event.(type) {
...
func (p *Node) process(tx *transaction.Transaction) {
...
func (p *Node) handleUnicityCertificateEvent(event interface{}) {
	switch event.(type) {
...
func (p *Node) handleBlockProposal(prop *blockproposal.BlockProposal) error {
...
func (p *Node) handleUnicityCertificate(uc *certificates.UnicityCertificate) error {
	...
	if p.pr == nil {
		...
		logger.Warning("Reverting state tree. UC IR hash: %X, proposal hash %X", uc.InputRecord.Hash, p.pr.prevHash)
...
func (p *Node) finalizeBlock(transactions []*transaction.Transaction, uc *certificates.UnicityCertificate) {
...
func (p *Node) handleT1TimeoutEvent() {
...
func (p *Node) sendBlockProposal() error {
...
func (p *Node) sendCertificationRequest() error {
	...
	req := &p1.P1Request{
		SystemIdentifier: systemIdentifier,
		NodeIdentifier:   nodeId.String(),
		RootRoundNumber:  p.pr.roundNumber,
	...
	err = req.Sign(p.configuration.signer)
	if err != nil {
		return err
	}
	...
	logger.Info("Sending block certification request event")
