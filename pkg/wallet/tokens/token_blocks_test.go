...
func TestTokensProcessBlock_syncToUnit(t *testing.T) {
	...
	require.NoError(t, w.syncToUnit(context.Background(), &submittedTx{id, tx}))
...
func TestTokensProcessBlock_syncToUnit_timeout(t *testing.T) {
	...
	require.ErrorContains(t, w.syncToUnit(context.Background(), &submittedTx{id, createTx(id, blockNr)}), "did not confirm all transactions, timed out: 1")
}
