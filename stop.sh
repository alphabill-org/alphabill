...
function stop() {
  ...
  case $1 in
    ...
    tokens)
      ...
      ;;
    programs)
      program="build/alphabill programs"
      ;;
    root)
...
usage() { echo "Usage: $0 [-h usage] [-a stop all] [-r stop root] [-p stop partition: money, vd, tokens, programs] [-b stop backend: money, tokens]"; exit 0; }
