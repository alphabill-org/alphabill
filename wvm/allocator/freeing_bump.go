...
func readHeaderFromMemory(mem Memory, headerPtr uint32) (Header, error) {
...
func writeHeaderInto(header Header, mem Memory, headerPtr uint32) error {
	var (
		headerData   uint64
		occupiedMask uint64
	)
...
func (f *FreeingBumpHeapAllocator) Allocate(mem Memory, size uint32) (ptr uint32, err error) {
	if f.poisoned {
...
func (f *FreeingBumpHeapAllocator) Deallocate(mem Memory, ptr uint32) (err error) {
	if f.poisoned {
...
func bump(bumper *uint32, size uint32, mem Memory) (uint32, error) {
