...
func Test_ReputationBased_Update(t *testing.T) {
	...
	t.Run("invalid input: qc.ParentRound + 1 != qc.Round", func(t *testing.T) {
		...
		err := rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 5, ParentRoundNumber: 3}}, 6)
		...
		err = rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 1, ParentRoundNumber: 1}}, 2)
		require.EqualError(t, err, `not updating leaders because rounds are not consecutive {parent: 1, QC: 1, current: 2}`)
		require.Equal(t, slotIdx, rl.curIdx, "expected that round leader buffer index is not changed")

		err = rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 1, ParentRoundNumber: 2}}, 2)
	...
	t.Run("invalid input: qc.Round + 1 != currentRound", func(t *testing.T) {
		...
		err := rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 2, ParentRoundNumber: 1}}, 1)
		require.EqualError(t, err, `not updating leaders because rounds are not consecutive {parent: 1, QC: 2, current: 1}`)
		require.Equal(t, slotIdx, rl.curIdx, "expected that round leader buffer index is not changed")

		err = rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 2, ParentRoundNumber: 1}}, 2)
		require.EqualError(t, err, `not updating leaders because rounds are not consecutive {parent: 1, QC: 2, current: 2}`)
		require.Equal(t, slotIdx, rl.curIdx, "expected that round leader buffer index is not changed")

		err = rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 2, ParentRoundNumber: 1}}, 4)
	...
	t.Run("failing to elect leader because block loader fails", func(t *testing.T) {
		...
		rl := &ReputationBased{
			windowSize: 1,
			loadBlock:  func(round uint64) (*ab_consensus.BlockData, error) { return nil, expErr },
		}
		slotIdx := rl.curIdx

		err := rl.Update(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{RoundNumber: 2, ParentRoundNumber: 1}}, 3)
	...
	t.Run("successfully electing leader", func(t *testing.T) {
		loadBlock := func(round uint64) (*ab_consensus.BlockData, error) {
			if round != 1 {
				return nil, fmt.Errorf("expected that round 1 block is requested, got request for round %d", round)
			}
			return &ab_consensus.BlockData{Author: signerAkey}, nil
		}
		...
		qc := &ab_consensus.QuorumCert{
			VoteInfo:   &certificates.RootRoundInfo{RoundNumber: 2, ParentRoundNumber: 1},
...
func Test_ReputationBased_GetLeaderForRound(t *testing.T) {
	...
	noLoadBlockCalls := func(round uint64) (*ab_consensus.BlockData, error) {
...
func Test_ReputationBased_electLeader(t *testing.T) {
	...
	t.Run("loading block fails with error", func(t *testing.T) {
		...
		id, err := rl.electLeader(&ab_consensus.QuorumCert{})
		require.ErrorIs(t, err, expErr)
	...
	t.Run("loading block fails - nil block is returned", func(t *testing.T) {
		rl := &ReputationBased{
			...
			loadBlock: func(round uint64) (*ab_consensus.BlockData, error) { return nil, nil },
		}
		id, err := rl.electLeader(&ab_consensus.QuorumCert{VoteInfo: &certificates.RootRoundInfo{ParentRoundNumber: 3}})
	...
	t.Run("single signer will be excluded and thus empty set to select from", func(t *testing.T) {
		rl := &ReputationBased{
			...
			excludeSize: 1,
			loadBlock: func(round uint64) (*ab_consensus.BlockData, error) {
				return &ab_consensus.BlockData{Author: "signer"}, nil
			},
		}
		id, err := rl.electLeader(&ab_consensus.QuorumCert{
			VoteInfo:   &certificates.RootRoundInfo{ParentRoundNumber: 3},
	...
	t.Run("exclude two last authors", func(t *testing.T) {
		...
		id, err := rl.electLeader(&ab_consensus.QuorumCert{
			VoteInfo:   &certificates.RootRoundInfo{RoundNumber: 5, ParentRoundNumber: 4},
...
func Test_NewReputationBased(t *testing.T) {
	...
	t.Run("invalid windowSize: 0", func(t *testing.T) {
		lr, err := NewReputationBased([]peer.ID{"A", "B"}, 0, 1, func(round uint64) (*ab_consensus.BlockData, error) { return nil, nil })
	...
	t.Run("success", func(t *testing.T) {
		...
		bloader := func(round uint64) (*ab_consensus.BlockData, error) { return nil, blErr }
...
func Test_ReputationBased(t *testing.T) {
	...
	blockStore := map[uint64]*ab_consensus.BlockData{}
	// to save the block to blockStore
	storeBlock := func(bd *ab_consensus.BlockData) error {
	...
	processRound := func(rl *ReputationBased, round uint64) error {
		...
		qc := &ab_consensus.QuorumCert{
			VoteInfo:   &certificates.RootRoundInfo{RoundNumber: round - 1, ParentRoundNumber: round - 2},
		...
		storeBlock(&ab_consensus.BlockData{
			Round:  round,
