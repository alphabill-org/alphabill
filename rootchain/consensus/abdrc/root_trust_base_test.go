...
func generateDummyValidatorMap(nofValidators uint64) map[string]abcrypto.Verifier {
	validators := make(map[string]abcrypto.Verifier)
...
func generateSignersAndVerifiers(nofValidators uint64) (map[string]abcrypto.Signer, map[string]abcrypto.Verifier) {
...
func createSignatures(t *testing.T, hash []byte, signers map[string]abcrypto.Signer) map[string][]byte {
	t.Helper()
...
func TestNewRootClusterVerifier(t *testing.T) {
	type args struct {
		keyMap    map[string]abcrypto.Verifier
		threshold uint32
	}
	tests := []struct {
		name       string
...
func TestRootNodeTrustBase_VerifyQuorumSignatures(t *testing.T) {
	...
	qc := &drctypes.QuorumCert{VoteInfo: voteInfo, LedgerCommitInfo: commitInfo, Signatures: signatures}
...
func TestRootNodeTrustBase_VerifySignature(t *testing.T) {
	...
	roundInfo := &drctypes.RoundInfo{RoundNumber: 1, Timestamp: 11111, CurrentRootHash: []byte{0, 1, 3}}
